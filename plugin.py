...
class TimeRuleConfig(PluginConfigBase):
    ...
    model_name: str = Field(default="", description="该时段使用的模型名。填法：模型配置页里你自己给模型起的名字（[[models]] 的 name 字段），如 deepseek-v4-flash，须一字不差；留空=本条规则无效", json_schema_extra={"label": "模型名", "placeholder": "如 deepseek-v4-flash", "hint": "填模型配置页 name 字段的名字，不是官方 API 编号"})
    task_names: List[str] = Field(default_factory=lambda: ["replyer"], description="生效任务：只能填 replyer（本插件仅作用于回复生成链路，官方钩子不支持给 planner 等其他任务指定模型，填了不生效）", json_schema_extra={"label": "生效任务（仅 replyer 有效）", "hint": "添加项目后填 replyer；填其他任务名无效"})
    days_of_week: List[str] = Field(default_factory=list, description="生效星期：添加项目后每个项目填一个词——weekday=周一-周五；weekend=周六-周日；或 mon/tue/wed/thu/fri/sat/sun（中文 周一-周日 也可以）；列表留空=每天生效。时区按本规则时区计算", json_schema_extra={"label": "生效星期（留空=每天）", "hint": "每项填 weekday / weekend / mon-sun / 周一-周日 之一"})
